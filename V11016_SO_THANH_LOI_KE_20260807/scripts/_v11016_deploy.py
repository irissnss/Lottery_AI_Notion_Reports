# -*- coding: utf-8 -*-
"""V11016 — deploy: SỐ THÀNH LỜI KỂ (L-A) + NGƯỠNG TỰ QUYẾT (L-B).
...
print("=" * 92)
print("  V11016 DEPLOY — số thành lời kể (L-A) + ngưỡng tự quyết (L-B)")
print("=" * 92)
...
sftp = cli.open_sftp()
r(f"cp {R}/{TEP} {R}/backups/gpt_analyzer.py.v11016_pre 2>/dev/null; "
  f"mkdir -p {R}/backups")
...
for m in ("MB", "MT", "MN"):
    ...
    ra[m] = {
        ...
        "nguong_tu_quyet": "NG\u01af\u1ee0NG T\u1ef0 QUY\u1ebeT" in cp,
...
print(f"  Gỡ về: cp {R}/backups/gpt_analyzer.py.v11016_pre {R}/{TEP} && systemctl restart lottery")
print(f"  Gỡ về local: backups/v11016_pre/gpt_analyzer.py.pre "
      f"(md5 f8b428ece9d90181642806e095ced195)")
print("=" * 92)
cli.close()
