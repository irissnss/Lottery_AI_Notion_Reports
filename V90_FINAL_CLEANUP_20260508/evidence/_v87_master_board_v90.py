...
def _settings_block(c: sqlite3.Connection) -> dict[str, Any]:
    """V88: app_settings table inventory. Sensitive categories (ai_keys) redacted in payload."""
    SENSITIVE_CATEGORIES = {"ai_keys"}
    rows = []
    by_cat: dict[str, int] = {}
    try:
    ...
    return {"total": len(rows), "by_category": by_cat, "rows": rows, "sensitive_categories_redacted": list(SENSITIVE_CATEGORIES)}
...
def _migrations_block() -> list[dict]:
    """V89: Inventory migration_*.py files in web/backend/ with first docstring line."""
    migrations_dir = ROOT / "web" / "backend"
    out = []
    for f in sorted(migrations_dir.glob("migration_*.py")):
        try:
            text = f.read_text(encoding="utf-8", errors="replace")
            doc_match = re.search(r'"""([^"]{10,500})"""', text, re.DOTALL)
            doc = (doc_match.group(1).strip() if doc_match else "").split("\n")[0][:200]
            tables_changed = sorted(set(re.findall(r'CREATE TABLE\s+(?:IF NOT EXISTS\s+)?(\w+)', text)
                                       + re.findall(r'ALTER TABLE\s+(\w+)', text)))
            mtime = dt.datetime.fromtimestamp(f.stat().st_mtime, tz=VN).isoformat(timespec="seconds")
        except Exception:
            doc, tables_changed, mtime = "", [], ""
        out.append({
            "file": f.name,
            "size_kb": f.stat().st_size // 1024,
            "modified_at_vn": mtime,
            "summary": doc,
            "tables_changed": tables_changed[:10],
        })
    return out


def _live_cron_block() -> dict[str, Any]:
    """V89: Read APScheduler runtime jobs to get next_run_time + last_run hint from scheduler_logs."""
    sys.path.insert(0, str(ROOT / "web" / "backend"))
    out = {"jobs": [], "scheduler_active": False}
    try:
        import scheduler as _sched_mod
        sch = getattr(_sched_mod, "_scheduler", None)
        if sch is not None and getattr(sch, "running", False):
            out["scheduler_active"] = True
            now_vn = dt.datetime.now(VN)
            for job in sch.get_jobs():
                ...
                if next_run is not None:
                    try:
                        seconds_to_next = int((next_run - now_vn).total_seconds())
                    except Exception:
                        seconds_to_next = None
                out["jobs"].append({
                    "id": job.id,
                    "name": getattr(job, "name", "") or job.id,
                    "next_run_at": next_iso,
                    "seconds_to_next": seconds_to_next,
                    "trigger": str(getattr(job, "trigger", "")),
                })
    except Exception as e:
        out["error"] = f"scheduler not loaded: {type(e).__name__}: {e}"

    # Last-run hint from scheduler_logs (most recent log per job_name)
    try:
        c = _conn()
        rows = list(c.execute("""
            SELECT job_name, MAX(created_at) AS last_seen, COUNT(*) AS log_count
            FROM scheduler_logs WHERE job_name IS NOT NULL AND job_name != ''
            GROUP BY job_name ORDER BY last_seen DESC LIMIT 50
        """))
        out["last_run_log_hints"] = [
            {"job_name": r["job_name"], "last_seen": r["last_seen"], "log_count": r["log_count"]}
            for r in rows
        ]
    except sqlite3.OperationalError:
        out["last_run_log_hints"] = []
...
def _fu_audit_block() -> dict[str, Any]:
    """V89: Audit FU items - flag stale (status said DEPLOYED but no recent CHANGELOG mention)."""
    fu_path = ROOT / "docs" / "FOLLOW_UP_TRACKER.md"
    chg_path = ROOT / "CHANGELOG.md"
    if not (fu_path.exists() and chg_path.exists()):
        return {"audited": 0, "rows": []}
    fu_text = fu_path.read_text(encoding="utf-8", errors="replace")
    chg_text = chg_path.read_text(encoding="utf-8", errors="replace")
    fu_pat = re.compile(r'###\s*(FU-\d+)\s*[—\-:]?\s*([^\n]*)', re.MULTILINE)
    matches = list(fu_pat.finditer(fu_text))
    seen = set()
    rows = []
    for i, m in enumerate(matches):
        ...
        # Mention count in CHANGELOG
        chg_mentions = len(re.findall(rf'\b{fu_id}\b', chg_text))
        # Flag stale: claims DEPLOYED/DONE but no recent CHANGELOG mention
        is_stale = False
        flag_reason = ""
        if status.upper().startswith(("DEPLOYED", "DONE", "LIVE_PROVEN")) and chg_mentions == 0:
            is_stale = True
            flag_reason = "Status claims DEPLOYED/DONE but FU not mentioned in CHANGELOG"
        elif "WAIT" in status.upper() and chg_mentions >= 3:
            is_stale = True
            flag_reason = "Status WAIT but FU has 3+ CHANGELOG mentions (likely already advanced)"
        rows.append({
            "id": fu_id, "title": title[:120],
            "status": status[:80], "chg_mentions": chg_mentions,
            "is_stale": is_stale, "flag_reason": flag_reason,
        })
    rows.sort(key=lambda x: int(x["id"].split("-")[1]) if x["id"].startswith("FU-") else 0)
    stale_count = sum(1 for r in rows if r["is_stale"])
    return {
        "audited": len(rows), "stale_count": stale_count,
        "rows": rows,
        "audit_rules": [
            "Stale if DEPLOYED/DONE/LIVE_PROVEN but 0 CHANGELOG mentions",
            "Stale if WAIT but >= 3 CHANGELOG mentions (likely advanced)",
        ],
    }


def _phase_findings_block() -> list[dict]:
    """V89: Extract first paragraph (~200 chars) findings from each phase_checkpoint."""
    chk_dir = ROOT / "artifacts" / "phase_checkpoints"
    if not chk_dir.exists():
        return []
    out = []
...
def _decision_log_block() -> list[dict]:
    """V89: Parse DECISION_LOG.md for owner decisions."""
    path = ROOT / "docs" / "DECISION_LOG.md"
    if not path.exists():
        return []
    text = path.read_text(encoding="utf-8", errors="replace")
    rows = []
    for m in re.finditer(r'\|\s*(DEC-\d+)\s*\|\s*(\d{4}-\d{2}-\d{2})\s*\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|\s*(✅|❌|✔️|✖️)?\s*\|\s*(\w+)\s*\|\s*([^|]*)\|\s*([^|\n]*)', text):
        rows.append({
            "id": m.group(1),
            "date": m.group(2),
            "decision": m.group(3).strip()[:200],
            "context": m.group(4).strip()[:200],
            "confirmed": m.group(5) or "",
            "finality": m.group(6).strip(),
            "supersedes": m.group(7).strip(),
            "cross_ref": m.group(8).strip()[:120],
        })
    rows.sort(key=lambda r: int(r["id"].split("-")[1]) if r["id"].startswith("DEC-") else 0, reverse=True)
    return rows


def _governance_ledger_block() -> list[dict]:
    """V89: Parse CHANGELOG_GOVERNANCE_LEDGER.md (table rows)."""
    path = ROOT / "docs" / "CHANGELOG_GOVERNANCE_LEDGER.md"
    if not path.exists():
        return []
    text = path.read_text(encoding="utf-8", errors="replace")
    rows = []
    for line in text.splitlines():
        if not line.startswith("| 2026-") and not line.startswith("| 2025-"):
            continue
        ...
        rows.append({
            "date": parts[0][:10],
            "title": parts[1][:200] if len(parts) > 1 else "",
            "summary": parts[2][:240] if len(parts) > 2 else "",
            "rationale": parts[3][:200] if len(parts) > 3 else "",
        })
    rows.sort(key=lambda r: r.get("date") or "", reverse=True)
    return rows


def _backend_modules_block() -> list[dict]:
    """V90: list all .py files in web/backend/ with size + first docstring line."""
    ...
    for f in sorted(backend.glob("*.py")):
        try:
            ...
            doc, mtime = "", ""
        kind = "materializer" if f.name.startswith("_materialize_") else \
               "test_lane" if "_du_doan_test" in f.name else \
               "audit" if f.name.startswith("_audit_") or f.name.startswith("_diag_") else \
               "core"
        out.append({
            "file": f.name, "size_kb": f.stat().st_size // 1024,
            "modified_at_vn": (mtime or "")[:19], "kind": kind, "summary": doc,
        })
    return out


def _scripts_block() -> list[dict]:
    """V90: list scripts/ folder."""
    ...
    for f in sorted(scripts.glob("*.py")):
        try:
            ...
            doc = ""
        out.append({"file": f.name, "size_kb": f.stat().st_size // 1024, "summary": doc})
    return out


def _web_helpers_block() -> list[dict]:
    """V90: list web/*.py deploy/sync helpers."""
    web = ROOT / "web"
    out = []
    if not web.exists():
        return out
    for f in sorted(web.glob("*.py")):
        try:
            ...
            doc = ""
        kind = "deploy" if "deploy" in f.name else \
               "sync" if "sync" in f.name else \
               "vps" if "vps" in f.name else \
               "diag" if "diag" in f.name else \
               "test" if "test" in f.name or "trigger" in f.name else \
               "helper"
        out.append({"file": f.name, "size_kb": f.stat().st_size // 1024, "kind": kind, "summary": doc})
    return out


def _active_roadmaps_block() -> list[dict]:
    """V90: list docs/ACTIVE_ROADMAP_*.md files (workspace rule mention)."""
    docs = ROOT / "docs"
    out = []
    if not docs.exists():
        return out
    for f in sorted(docs.glob("ACTIVE_ROADMAP_*.md")):
        try:
            text = f.read_text(encoding="utf-8", errors="replace")
            title = ""
            status = "UNKNOWN"
        ...
        out.append({
            "file": f.name, "size_kb": f.stat().st_size // 1024,
            "title": title[:200], "status": status[:60],
        })
    return out


def _cursor_rules_block() -> list[dict]:
    """V90: list .cursor/rules/*.mdc files (workspace rules)."""
...
def build_payload() -> dict[str, Any]:
    """Return the full V87+V88+V89+V90 master board payload. READ-ONLY."""
    c = _conn()
    return {
        "schema_version": "v90_master_board_v4",
        ...
        # V89 extensions
        "migrations": _migrations_block(),
        "live_cron": _live_cron_block(),
        "fu_audit": _fu_audit_block(),
        "phase_findings": _phase_findings_block(),
        "decision_log": _decision_log_block(),
        "governance_ledger": _governance_ledger_block(),
        # V90 extensions
        "backend_modules": _backend_modules_block(),
        "scripts": _scripts_block(),
        "web_helpers": _web_helpers_block(),
        "active_roadmaps": _active_roadmaps_block(),
        "cursor_rules": _cursor_rules_block(),
        "owner_controls": {
            "promote_button": False, "rollback_button": False, "edit_button": False, "trigger_run_button": False,
            "note": "V87+V88+V89+V90 Master Index cố ý không có nút điều khiển. Mọi thay đổi đều cần owner OK ở session riêng + dossier.",
        },
    }
