# -*- coding: utf-8 -*-
"""V11018 — deploy: bảng đo BẦY ĐÀN (phép đo CƠ CHẾ của V11016) + API + panel + cron.

Ba tệp: `_v11018_ma_doi_chung.py` (mới) · `main.py` (thêm 1 endpoint admin) ·
...
TEP = ["web/backend/_v11018_ma_doi_chung.py",
       "web/backend/main.py",
...
CRON = ("5 19 * * * cd /root/Lottery_AI_Test/web/backend && "
        "/root/Lottery_AI_Test/venv/bin/python3 _v11018_ma_doi_chung.py "
        ">> /root/Lottery_AI_Test/logs/v11018_ma_doi_chung.log 2>&1")
...
print("=" * 92)
print("  V11018 DEPLOY — đối chứng ba nguồn ML × luật × bộ lọc (M-A)")
print("=" * 92)
...
for f in TEP:
    ...
    r(f"cp -n {R}/{f} {R}/backups/$(basename {f}).v11018_pre 2>/dev/null")
    sftp.put(f, f"{R}/{f}")
sftp.close()
lech = []
for f in TEP:
...
bd = r(f"cd {R} && {PY_VENV} -m py_compile web/backend/main.py "
       f"web/backend/_v11018_ma_doi_chung.py && echo OK")
...
out = r(f"cd {R}/web/backend && {PY_VENV} -c \""
        ...
        f"import _v11018_ma_doi_chung as M; c=M.compute(); v=M.view(); "
        f"sys.stdout=_t; "
        f"print('@@@'+json.dumps({{'compute':c,'ok':v.get('success'),"
        f"'an_toan':v.get('an_toan'),'nhom':v.get('theo_so_nguon'),"
        f"'ket':v.get('ket_luan')}}))\"", t=600)
kq = None
...
print(f"  compute()    : ghi {kq['compute']['so_dong']} dòng "
      f"(đến {kq['compute']['den_ngay']})")
print(f"  view()       : theo số nguồn={kq['nhom']} · kết luận={kq['ket']}")
...
if not du_co or kq["compute"]["so_dong"] < 500:
    print("  ✗ DỪNG — cờ an toàn thiếu hoặc ghi quá ít dòng")
    raise SystemExit(1)

# ── cron ──────────────────────────────────────────────────────────────────────
co_cron = r("crontab -l 2>/dev/null | grep -c '_v11018_ma_doi_chung'")
if co_cron.strip() == "0":
    r(f"(crontab -l 2>/dev/null; echo '{CRON}') | crontab -")
n_cron = r("crontab -l 2>/dev/null | grep -c '_v11018_ma_doi_chung'")
...
admin = r("curl -s -o /dev/null -w '%{http_code}' "
          "http://127.0.0.1:8000/api/admin/ma-doi-chung")
nostore = r("curl -s -D- -o /dev/null http://127.0.0.1:8000/api/admin/ma-doi-chung "
...
print(f"  Gỡ về: for f in {' '.join(x.split('/')[-1] for x in TEP)}; do "
      f"cp {R}/backups/$f.v11018_pre <đúng đường dẫn>; done && systemctl restart lottery")
print(f"  Gỡ cron: crontab -l | grep -v _v11018_ma_doi_chung | crontab -")
print("=" * 92)
cli.close()
